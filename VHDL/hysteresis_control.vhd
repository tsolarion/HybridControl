...
-- =================== STATES ====================================================
type hysteresis_state is (IDLE,PWM_CONTROL,WAIT_STEP_UP, NEW_STEP_UP,NEW_STEP_DOWN, WAIT_STEP_DOWN,FIRST_DOWN, SECOND_UP, PHASE_SHIFT, THIRD_UP, THIRD_DOWN); --! State machine for hysteresis control 
-- =================== SIGNALS ===================================================
-- State machine 
...
begin	
	...
	statemachine_logic: process(hyst_state_s,iset_s,iset_tot_s,iset_tot_delayed_s,ierr_s, i_upper_s, i_lower_s,
								...
								up_iset_s,up_ierr_s,up_vc_s,down_iset_s,down_ierr_s,down_vc_s,hyst_enable_s, 
		...
		case hyst_state_s is 
			...
			when PWM_CONTROL => 
				...
				if hyst_enable_s = '1' and (up_iset_s='1' or up_ierr_s='1' or up_vc_s='1') then 
					...
					-- end if; 					
					i_upper_next_s <= iset_s + H0_C; 
					i_lower_next_s <= iset_s - H0_C; 		
					hyst_next_s <= '1'; 
			...
			when THIRD_DOWN => 
				if imeas_s < i_lower_s then
					...
					if cnt_cycle_s = N_CYCLE_REST_G then -- went through all cycles 
						...
						hyst_state_next_s <= THIRD_UP; -- do one more cycle
