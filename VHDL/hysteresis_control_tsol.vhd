...
constant CNT_RISE : integer := 35000; -- MAX counter for ripple. This needs to be properly calculated, for all operating points 
...
--Tsol:
	...
	statemachine_logic: process(hyst_state_s,iset_s,iset_tot_s,iset_tot_delayed_s,ierr_s, i_upper_s, i_lower_s,
								imeas_s,t1_start_s,hss_bound_i,deltaH_i,hyst_s,deltaH_ready_i,phase_shift_en,
		...
		case hyst_state_s is 
			...
			when PWM_CONTROL => 
				...
				if hyst_enable_s = '1' and (up_iset_s='1' or up_ierr_s='1' or up_vc_s='1') then 
					...
					-- end if; 					
					i_upper_next_s <= iset_s + (H0_C - Hadj_Rise_1st_G); 
					i_lower_next_s <= iset_s - (H0_C - Hadj_Fall_1st_G); 			
					hyst_next_s <= '1'; 
			...
			when NEW_STEP_UP => 
				...
				if imeas_s  >= i_upper_s then 
					--hyst_state_next_s <= FIRST_DOWN; -- EDIT TSOL
            ...
            when DELAY_D1 => 
				-- waiting for the time delay of the current
                if dly_cnt_s  < CNT_DELAY  AND dly_cnt_s  < CNT_RISE  then 
					dly_cnt_next_s <= dly_cnt_s +1; 
				else 
					dly_cnt_next_s <= 0; 
					hyst_state_next_s <= NEW_STEP_UP; 
			...
			when SECOND_UP => 
				t2_start_next_s <= '0'; 
				if imeas_s  >= i_upper_s then 
					--if phase_shift_en = '1' then -- all modules first NEW_STEP mode passed 			
						hyst_state_next_s <= DELAY_3; 
						i_upper_next_s <= iset_s + to_integer(hss_bound_i) - 570;
						i_lower_next_s <= iset_s - to_integer(hss_bound_i) +60; 
					--else 
					--	hyst_state_next_s <= FIRST_DOWN; 
					--	t1_start_next_s <= '1';
					--end if; 
