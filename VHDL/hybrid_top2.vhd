...
-- PACKAGE std16_arr_pkg IS
    -- type array_std16 is array (natural range <>) of std_logic_vector (15 downto 0);
-- END; 
...
--! @details Introduces usagage of master and a multiple slaves 
entity hybrid_top is
	generic( 	CMAX_G : integer := 1666; --! Maximum counter value of PWM (determines PWM frequency) =>  ~60 KHz with 100 MHz clock 
				...
				DELTA_I_REF_G 		: integer := 25*(2**5)*2; --! minimum set current change (25 A) for entering hysteresis mode 
				DELTA_I_THR_G 		: integer := 40*(2**5)*2; --! minimum current difference (25 A) between measured and set current for entering hysteresis mode 
				DELTA_VC_G			: integer := 100*(2**5); --! minimum VC change change (100 V) for entering hysteresis mode 
				D_IOUT_MAX_G		: integer := 10*(2**5); --! Maximum current ripple after first rise (here 5A) 
				HYST_COND_SEL_G		: std_logic_vector(2 downto 0):= "111"; --! Enable conditions for entering hysteresis: 2: vc, 1: ierr, 0: iset 
				N_CYCLE_REST_G		: integer := 3; --! Number of cycles controller stays in Hysterssis after phaseshift 
				-- Variable L points
	...
	port(
		...
		vbusl_i     	: in  unsigned(MEAS_V_DATAWIDTH_G-1 downto 0); --! V2 measured voltage (negative)	
...
-- ================== COMPONENTS =================================================
	...
	generic( CMAX_G 			: integer := 1666; --! Maximum counter value of PWM (determines PWM frequency) =>  ~60 KHz with 100 MHz clock   
			...
			-- PI settings 
			ANTI_WINDUP_G: integer 				:= 100*(2**5); --! maximum error for integration active 
...
-- =================== STATES ====================================================
	...
	generic map(CMAX_G 				=> CMAX_G, 
				...
				MY_NUMBER_G 		=> 0, -- indice of current slave: 0 indicates master 
	...
	--generate for loop (here only one slave) 
	SLAVE_MODULE: 
